...
namespace Presentations
{
    public partial class QuanLyThiDau : UserControl
    {
        ...
        private TranDau GetValue_TranDau()
        {
            ...
            if (mode == 1 || mode == 2)
            {
                int ma = dataGridView1_thidau.Rows.Count;
                if (ma > 0)
                {
                    a.Matrd = dataGridView1_thidau.Rows[ma - 1].Cells[0].Value.ToString();
                }
                else
                {
                    a.Matrd = "";
        ...
        {
            mode = 1;
            if (!DataUtils.KiemTra(cbbMaDB1.Text, cbbMaDB2.Text, cbbMaSan.Text, cbbVong.Text, cbbPhutThiDau.Text, cbbGioThiDau.Text))
            {
        ...
        private void btXoaThiDau_Click(object sender, EventArgs e)
        {
            mode = 2;
            int flg = 0;
            foreach (DataGridViewRow item in dataGridView1_thidau.SelectedRows)
            {
                TranDau td = new TranDau();
                td.Matrd = item.Cells[0].Value.ToString();
                if (qltd.Delete_TranDau(td))
                {
                    flg = 1;
                }
                dataGridView1_thidau.Rows.RemoveAt(item.Index);
            }

            if (flg == 1)
            {
                MessageBox.Show("Xoá trận đấu thành công");
                this.LoadTranDau();
            }
            else
            {
        ...
        private RaSan GetValueRaSan()
        {
            ...
            if (mode == 1 || mode == 2)
            {
                int ma = dataGridView1_RaSan.Rows.Count;
                if (ma > 0)
                {
                    a.Mars = dataGridView1_RaSan.Rows[ma - 1].Cells[0].Value.ToString();
                }
                else
                {
                    a.Mars = "";
        ...
        {
            mode = 1;
            if (!DataUtils.KiemTra(cbbmacauthurasan.Text))
            {
                MessageBox.Show("Bạn chưa điền đầy đủ thông tin.");
            }
        ...
        private void btXoaRaSan_Click(object sender, EventArgs e)
        {
            mode = 2;
            int flg = 0;
            foreach (DataGridViewRow item in dataGridView1_RaSan.SelectedRows)
            {
                RaSan rs = new RaSan();
                rs.Mars = item.Cells[0].Value.ToString();
                if (qltd.Delete_RaSan(rs))
                {
                    flg = 1;
                }
                dataGridView1_RaSan.Rows.RemoveAt(item.Index);
            }

            if (flg == 1)
            {
                MessageBox.Show("Xoá ra sân thành công");
                this.Load_RaSan();
            }
            else
            {
