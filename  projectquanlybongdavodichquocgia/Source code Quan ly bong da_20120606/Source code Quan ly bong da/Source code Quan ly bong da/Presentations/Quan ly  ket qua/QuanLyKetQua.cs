...
namespace Presentations
{
    public partial class QuanLyKetQua : UserControl
    {
        ...
        private void btXoa_KQ_Click(object sender, EventArgs e)
        {
            ...
            {
                KetQua kq = new KetQua();
                kq.Ketqua = item.Cells[0].Value.ToString();
                if (qlkq.Delete_KetQua(kq))
                {
                    flg = 1;
                }
                dataGridView1_KQ.Rows.RemoveAt(item.Index);
            }
        ...
        private GhiBan GetValue_GhiBan()
        {
            ...
            if (mode == 1 || mode == 2)
            {
                int ma = dataGridView2_GhiBan.Rows.Count;
                if (ma > 0)
                {
                    gb.Mabt = dataGridView2_GhiBan.Rows[ma - 1].Cells[0].Value.ToString();
                }
                else
                {
                    gb.Mabt = "";
        ...
        {
            mode = 1;
            if (!DataUtils.KiemTra(cbbLoaiBanThang_GhiBan.Text, cbbMaCauThu_GhiBan.Text, cbbMaTrD_GhiBan.Text, ThoiDiemGhiBan.Text))
            {
                MessageBox.Show("Bạn chưa điền đầy đủ thông tin");
            }
        ...
        private void btXoa_GhiBan_Click(object sender, EventArgs e)
        {
            ...
            {
                GhiBan gb = new GhiBan();
                gb.Mabt = item.Cells[0].Value.ToString();
                if (qlkq.Delete_GhiBan(gb))
                {
                    flg = 1;
                }
                dataGridView2_GhiBan.Rows.RemoveAt(item.Index);
            }

            if (flg == 1)
            {
                MessageBox.Show("Xoá ghi bàn thành công");
                this.Load_GhiBan();
            }
            else
            {
        ...
        private Phat GetValue_Phat()
        {
            ...
            if (mode == 1 || mode == 2)
            {
                int ma = dataGridView2.Rows.Count;
                if (ma > 0)
                {
                    p.Maphat = dataGridView2.Rows[ma - 1].Cells[0].Value.ToString();
                }
                else
                {
                    p.Maphat = "";
        ...
        {
            mode = 1;
            if (!DataUtils.KiemTra(cbbLoaiThePhat.Text, cbbMaCT_Phat.Text, cbbMaTranDau_Phat.Text))
            {
        ...
        private void btXoa_Phat_Click(object sender, EventArgs e)
        {
            ...
            {
                Phat p = new Phat();
                p.Maphat = item.Cells[0].Value.ToString();
                if (qlkq.Delete_Phat(p))
                {
                    flg = 1;
                }
            ...
            if (flg == 1)
            {
                MessageBox.Show("Xoá phạt thành công");
                this.Load_Phat();
            }
            else
            {
